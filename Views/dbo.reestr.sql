...
SELECT 
	...
	, r.num
	, p.Name AS project
	, Okrug.shortName AS Okrug
	, raion.ShortName as ulitsa
	, /*[dbo].[ObjectFullAddress](a.ulitsa_unid,' ',1,0,0)*/ A.nazvanie_ulitsi
	,a.ulitsa_unid
	, h.HOUSENUM AS dom
	, h.BUILDNUM AS korpus
	, h.STRUCNUM AS stroenie
	...
	,[dbo].[dom_korp_str_toString](h.HOUSENUM,h.BUILDNUM,h.STRUCNUM,es.NAME,Ss.NAME,0) AS dom_korp_str2
...
FROM            
	 ...
	 inner join fias.House AS h ON h.HOUSEGUID=a.House_ID
	 inner join dbo.AdminArea Okrug ON Okrug.id=A.okrug_id
	 inner join dbo.AdminArea as Raion  ON Raion.id=A.AdminArea_ID
	 ...
	 LEFT OUTER JOIN fias.EstateStatus es  ON h.ESTSTATUS=es.ESTSTATID
	 LEFT OUTER JOIN [fias].[StructureStatus] ss  ON h.STRSTATUS=ss.STRSTATID
	 INNER JOIN Program p on p.ID=r.program_ID
WHERE 
not a.okrug_id in ('47E6B906-5978-46B4-902D-35D44DB63631')
and 
getdate() between h.STARTDATE and h.ENDDATE
...
CREATE TRIGGER [insteadTrig_INSERT] ON [dbo].[reestr]
INSTEAD OF INSERT
AS
BEGIN
SET NOCOUNT ON
...
CREATE TRIGGER [insteadTrig_UPDATE] ON [dbo].[reestr]
INSTEAD OF update
AS
BEGIN
SET NOCOUNT ON
...
FROM 
	inserted
	inner join reestr_t r on inserted.kod=r.kod
	inner join Program p on p.Name=inserted.project
      ...
      Begin Tables = 
         ...
         Begin Table = "zam1"
            Begin Extent = 
               ...
               Right = 913
            End
            DisplayFlags = ', 'SCHEMA', N'dbo', 'VIEW', N'reestr'
GO

EXEC sys.sp_addextendedproperty N'MS_DiagramPane2', N'280
            TopColumn = 0
         End
         Begin Table = "dlina"
            Begin Extent = 
   ...
   Begin DataPane = 
      ...
      Begin ColumnWidths = 246
         ...
         Width = 1500
         Width = 2355
', 'SCHEMA', N'dbo', 'VIEW', N'reestr'
GO

EXEC sys.sp_addextendedproperty N'MS_DiagramPane3', N'         Width = 1500
         Width = 1500
         Width = 1500
         Width = 1500
         Width = 1770
...
EXEC sys.sp_addextendedproperty N'MS_DiagramPane4', N'         Width = 1500
         Width = 1500
         Width = 1500
         Width = 1500
         Width = 1500
...
EXEC sys.sp_addextendedproperty N'MS_DiagramPaneCount', 4, 'SCHEMA', N'dbo', 'VIEW', N'reestr'
GO